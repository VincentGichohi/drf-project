...
class StreamPlatformTestCase(APITestCase):
    #To login the user in order for them to access the platform
    def setUp(self):
...
class WatchListTestCase(APITestCase):
    ...
    def test_watchlist_list(self):
        response = response = self.client.get(reverse('movie-list'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_watchlist_ind(self):
        response = response = self.client.get(reverse('movie-detail', args=(self.watchlist.id,)))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class ReviewTestCase(APITestCase):
    ...
    def setUp(self):
        #A setup to create a user
        self.user = User.objects.create_user(username='example', password='Password@123')
        self.token = Token.objects.get(user__username=self.user)
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token.key)

        #A setup to create a stream
        self.stream = models.StreamPlatForm.objects.create(name="Netflix",
                                                            about="#1 Platform",
                                                    website="https://www.netflix.com")
        #A setup to create a movie/watchlist
        self.watchlist = models.WatchList.objects.create(platform=self.stream, title='Example Movie',
                                                            storyline='Example Movie', active=True)
        self.watchlist2 = models.WatchList.objects.create(platform=self.stream, title='Example Movie',
                                                            storyline='Example Movie', active=True)
        #A setup for the review made before updating
    ...
    #Reviewing when authorized as the user created above
    ...
    #Testing reviewing when unauthorized
